...
export function Navbar() {
  ...
  return (
    <>
        ...
        <div className="container-main">
          <div
            className={`flex items-center gap-4 ${
              isHomepage
                ? `mt-3 h-[4.95rem] rounded-[30px] border px-4 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur-2xl ${
                    scrolled
                      ? "border-stone-200/90 bg-white/90"
                      : "border-white/50 bg-white/72"
                  }`
                : "h-[4.25rem]"
            }`}
          >
            ...
            <form onSubmit={handleSearch} className="mx-2 hidden flex-1 md:flex">
              <div className="relative w-full">
                ...
                <input
                  ...
                  placeholder={isHomepage ? "Rechercher caftans, deco, cadeaux..." : settings.searchPlaceholder || "Rechercher..."}
                  className={`h-11 w-full pl-11 pr-24 text-sm outline-none transition-all placeholder:text-muted-foreground ${
                    isHomepage
                      ? "rounded-full border border-stone-200/90 bg-[#fbfaf7]/92 shadow-[inset_0_1px_2px_rgba(15,23,42,0.04)] backdrop-blur focus:border-[#0d7a5e]/35 focus:ring-4 focus:ring-[#0d7a5e]/10 focus:bg-white"
                      : "rounded-xl border border-border/80 bg-white/80 shadow-inner-sm backdrop-blur focus:border-brand-500/60 focus:ring-3 focus:ring-brand-500/10 focus:bg-white dark:bg-card/80"
                  }`}
                  style={{ boxShadow: "inset 0 1px 3px rgba(15,23,42,0.06)" }}
                />
                <button
                  type="submit"
                  className={`absolute right-1.5 top-1.5 inline-flex h-8 items-center gap-1.5 px-3 text-xs font-bold text-white transition-all ${
                    isHomepage
                      ? "rounded-full bg-[#0d7a5e] hover:bg-[#0b6a51] hover:shadow-[0_12px_24px_rgba(13,122,94,0.22)]"
                      : "rounded-lg bg-brand-700 hover:bg-brand-600 hover:shadow-brand"
                  }`}
                >
                  <Sparkles className="h-3 w-3" />
                  {isHomepage ? "Explorer" : "Rechercher"}
                </button>
              </div>
            </form>
      ...
      <div className={isHomepage ? "h-[6.15rem]" : "h-[4.25rem]"} />
