...
import { X, ShoppingBag, Plus, Minus, Trash2, Tag, ArrowRight, ShieldCheck, Sparkles, Truck } from "lucide-react";
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {items.length > 0 && (
              <div className="relative border-b border-gold-200/20 bg-white/55 px-6 py-3 dark:border-gold-800/15 dark:bg-white/5">
                <p className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500 dark:text-white/60">
                  <Sparkles className="h-3.5 w-3.5 text-[#c89b3c]" />
                  Selection premium prete pour le checkout
                </p>
              </div>
            )}
              ...
              <div className="relative px-6 py-3.5 border-b border-gold-200/20 dark:border-gold-800/15 bg-brand-50/50 dark:bg-brand-900/20">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5 text-xs font-medium text-brand-700 dark:text-brand-400">
                    ...
                    Plus que {formatPrice(Math.max(0, freeShippingThreshold - subtotal))} pour debloquer la livraison offerte
                  </div>
              ...
              {items.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground mb-6 max-w-xs leading-relaxed">
                    Explorez la boutique pour retrouver vos prochains coups de coeur et relancer le parcours d&apos;achat.
                  </p>
              ...
              <div className="relative border-t border-gold-200/30 dark:border-gold-800/20 bg-white/60 dark:bg-card/60 backdrop-blur px-6 py-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex items-center gap-2 rounded-xl bg-muted/60 px-3 py-2.5">
                    ...
                    <span className="text-xs font-semibold text-foreground/70">Paiement securise</span>
                  </div>
                  <div className="flex items-center gap-2 rounded-xl bg-muted/60 px-3 py-2.5">
                    <Truck className="h-4 w-4 text-gold-600 flex-shrink-0" />
                    <span className="text-xs font-semibold text-foreground/70">Livraison suivie</span>
                  </div>
                </div>
                ...
                {coupon && (
                  <div className="flex items-center justify-between rounded-xl border border-brand-200 dark:border-brand-800 bg-brand-50 dark:bg-brand-900/20 p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-brand-700 dark:text-brand-400">{coupon.code} applique</span>
                    </div>
