...
export function ProductsClient({ categories, maxPrice, searchParams: initialSearchParams }: Props) {
  ...
  const loadedIdsRef = useRef<Set<string>>(new Set()); // Track loaded product IDs
  ...
  const fetchProducts = useCallback(async (reset = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const newProducts = fetched.filter((product) => !loadedIdsRef.current.has(product.id));
        // Mark new products as loaded
        newProducts.forEach((product) => loadedIdsRef.current.add(product.id));
        setProducts((prev) => (reset ? fetched : [...prev, ...newProducts]));
