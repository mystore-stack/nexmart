...
import { Settings, Eye, EyeOff, Save, X, Sparkles } from "lucide-react";
      ...
      if (data.success && data.data) {
        ...
        if (foundSection) {
          ...
          setFormData({
            ...
            themeSettings: {
              theme: foundSection.themeSettings?.theme || "Default",
              banner: foundSection.themeSettings?.banner || "",
              background: foundSection.themeSettings?.background || "",
              decorations: foundSection.themeSettings?.decorations || "",
              cta: foundSection.themeSettings?.cta || "",
              accentColor: foundSection.themeSettings?.accentColor || "#d4a437",
              products: foundSection.themeSettings?.products || "",
              categories: Array.isArray(foundSection.themeSettings?.categories)
                ? foundSection.themeSettings.categories
                : [],
              autoActivate: foundSection.themeSettings?.autoActivate ?? true,
              startDate: foundSection.themeSettings?.startDate || "",
              endDate: foundSection.themeSettings?.endDate || "",
            },
          });
        }
      }
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-3xl p-6 w-full max-w-3xl shadow-luxury max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSave} className="space-y-4 text-xs">
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <select
                        value={formData.themeSettings.theme}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, theme: e.target.value } })}
                        className="input w-full"
                      >
                    ...
                    <div>
                      <label className="font-bold block mb-1">Banner</label>
                      <input
                        type="text"
                        value={formData.themeSettings.banner}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, banner: e.target.value } })}
                        className="input w-full"
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={formData.themeSettings.background}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, background: e.target.value } })}
                        className="input w-full"
                    ...
                    <div>
                      <label className="font-bold block mb-1">Decorations</label>
                      <textarea
                        value={formData.themeSettings.decorations}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, decorations: e.target.value } })}
                        className="input w-full min-h-[60px]"
                        rows={2}
                      />
                    </div>
                    ...
                    <div>
                      <label className="font-bold block mb-1">CTA</label>
                      <input
                        type="text"
                        value={formData.themeSettings.cta}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, cta: e.target.value } })}
                        className="input w-full"
                      />
                    </div>

                    <div>
                      <label className="font-bold block mb-1">Accent Color</label>
                      <input
                        type="color"
                        value={formData.themeSettings.accentColor}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, accentColor: e.target.value } })}
                        className="input w-full h-10 p-1"
                      />
                    </div>

                    <div>
                      <label className="font-bold block mb-1">Products</label>
                      <textarea
                        value={formData.themeSettings.products}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, products: e.target.value } })}
                        className="input w-full min-h-[60px]"
                        rows={2}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={formData.themeSettings.categories?.join(", ") || ""}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, categories: e.target.value.split(",").map((value) => value.trim()).filter(Boolean) } })}
                        className="input w-full"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="font-bold block mb-1">Auto Activate</label>
                        <button
                          type="button"
                          onClick={() => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, autoActivate: !formData.themeSettings.autoActivate } })}
                          className={`w-12 h-6 rounded-full transition-colors ${
                            formData.themeSettings.autoActivate ? "bg-brand-700" : "bg-muted"
                          }`}
                        >
                          <div className={`w-5 h-5 rounded-full bg-white transition-transform ${
                            formData.themeSettings.autoActivate ? "translate-x-6" : "translate-x-0.5"
                          }`} />
                        </button>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="date"
                          value={formData.themeSettings.startDate || ""}
                          onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, startDate: e.target.value } })}
                          className="input w-full"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={formData.themeSettings.endDate || ""}
                        onChange={(e) => setFormData({ ...formData, themeSettings: { ...formData.themeSettings, endDate: e.target.value } })}
                        className="input w-full"
                      />
                    </div>
                  </div>
