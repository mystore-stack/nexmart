...
// ═══════════════════════════════════════════════════════════════
// Additional email functions for automation
// ═══════════════════════════════════════════════════════════════
export async function sendCartAbandonmentEmail(
  ...
  await sendEmail({
    ...
    html: baseTemplate(`
      <h1 style="font-size:28px;font-weight:700;color:${BRAND_NAVY};margin:0 0 8px;line-height:1.2;">
        Votre panier vous attend, ${name.split(" ")[0]}!
      </h1>
      <p style="color:#64748b;font-size:15px;margin:0 0 4px;">Retrouvez vos articles favoris sur NexMart.</p>
      ${goldDivider}
      <p style="color:#334155;font-size:15px;line-height:1.7;margin:0 0 24px;">
        Nous avons remarqué que vous avez laissé des articles dans votre panier. 
        Ne manquez pas cette occasion de les acquérir!
      </p>
      ${ctaButton("🛍️ Récupérer mon panier", recoveryUrl)}
    `),
  });
}
...
export async function sendStockAlertEmail(
  ...
  await sendEmail({
    to: email,
    subject: "🔔 Revenu en stock!",
    html: baseTemplate(`
      <h1 style="font-size:28px;font-weight:700;color:${BRAND_NAVY};margin:0 0 8px;line-height:1.2;">
        Bonne nouvelle, ${name.split(" ")[0]}!
      </h1>
      <p style="color:#64748b;font-size:15px;margin:0 0 4px;">L'article que vous attendez est de retour.</p>
      ${goldDivider}
      <p style="color:#334155;font-size:15px;line-height:1.7;margin:0 0 24px;">
        <strong>${productName}</strong> est de nouveau disponible en stock.
        Commandez-le maintenant avant qu'il ne soit plus disponible!
      </p>
      ${ctaButton("🛍️ Commander maintenant", productUrl)}
    `),
  });
}
...
export async function sendDailySalesReportEmail(
  email: string,
  reportData: { totalSales: number; orderCount: number; topProducts: any[] }
) {
  await sendEmail({
    to: email,
    ...
    html: baseTemplate(`
      ...
      ${goldDivider}
      <p style="color:#334155;font-size:15px;line-height:1.7;margin:0 0 24px;">
        Ventes totales: <strong>${reportData.totalSales} MAD</strong><br>
        Nombre de commandes: <strong>${reportData.orderCount}</strong>
      </p>
      ${ctaButton("📈 Voir le détail", `${APP_URL}/admin/analytics`)}
    `),
  });
}

export async function getEmailStatistics(organizationId?: string, startDate?: Date, endDate?: Date) {
