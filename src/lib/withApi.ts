...
export interface ApiContext {
  ...
  session?: Awaited<ReturnType<typeof import("./auth-api").requireAuthInternal>> | null;
}

async function normalizeRouteParams(params: RouteParams | Promise<RouteParams>) {
...
export type NextRouteContext = any;
...
export function withApi<T = unknown>(
  ...
  return async (req: NextRequest, routeContext: any) => {
    const routeParams = routeContext?.params as Promise<NextRouteParams> | undefined;
    const params = routeParams ? await normalizeRouteParams(routeParams) : undefined;
