...
export class ChurnPredictor {
  ...
  private static async calculateFeatures(user: any, organizationId: string) {
    ...
    const engagementScore = await this.calculateEngagementScore(user.id, organizationId);

    // Support tickets count
    const supportTickets = await this.countSupportTickets(user.id);

    // Returns count
    const returns = await this.countReturns(user.id);
  ...
  static async predictForOrganization(organizationId: string) {
    // Get users through organization relation or filter by orders/products in organization
