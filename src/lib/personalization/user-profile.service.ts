...
export class UserProfileService {
  ...
  static async getOrCreateProfile(userId: string, organizationId: string) {
    ...
    if (!profile) {
      profile = await prisma.userProfile.create({
        data: {
          userId,
          preferences: { preferredCategories: [], preferredPriceRange: {}, preferredBrands: [] },
          behavior: {},
          demographics: {},
        },
      });
  ...
  static async updateProfileFromEvent(
    ...
    if (eventData.categoryId) {
      await this.updateCategoryPreferences(profile.userId, eventData.categoryId);
    }
    ...
    await this.recalculateScores(updatedProfile.userId);
  ...
  private static async updateProductPreferences(profileId: string, productId: string) {
    ...
    preferredCategories.sort((a: any, b: any) => b.count - a.count);
    const topCategories = preferredCategories.slice(0, 10);

    await prisma.userProfile.update({
      where: { userId: profileId },
      data: { preferences: { ...preferences, preferredCategories: topCategories } },
    });
  }
  ...
  private static async recalculateScores(profileId: string) {
    ...
    // Simplified scoring based on preferences
    const preferences = profile.preferences as any;
    const engagementScore = preferences.engagementScore || 0;
    const purchaseScore = preferences.purchaseScore || 0;
    const activityScore = preferences.activityScore || 0;
