...
export async function safeFetchJSON<T = any>(
  ...
  try {
    ...
    if (!response.ok) {
      // 401 Unauthorized is expected for unauthenticated users, log as info instead of error
      if (response.status === 401) {
        console.log("[safeFetchJSON] Unauthorized request (expected for unauthenticated users)");
      } else {
