// src/app/admin/intents/[id]/edit/page.tsx
'use client';
...
export default function EditIntentPage() {
  ...
  const [intent, setIntent] = useState(mockIntents[intentId] || mockIntents['1']);
  const [title, setTitle] = useState(intent.title);
  const [icon, setIcon] = useState(intent.icon);
  const [description, setDescription] = useState(intent.description);
  const [selectedCategories, setSelectedCategories] = useState<string[]>(intent.categories);
  const [selectedProducts, setSelectedProducts] = useState<string[]>(intent.products);
  const [color, setColor] = useState(intent.color);
  const [active, setActive] = useState(intent.active);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Edit Intent #{intentId}</h1>
            <p className="text-slate-600 mt-1">Modify smart intent for auto bundle generation</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm opacity-80">
              {active ? 'Intent is generating bundles' : 'Intent is paused'}
            </p>
          </div>
          <div className="text-right">
            <p className="text-sm">Auto Bundles Generated</p>
            <p className="font-bold text-lg">{intent.autoBundles}</p>
          </div>
        </div>
      </div>
