...
import {
  Save, Plus, Trash2, Edit, ArrowLeft, Gift,
...
export default function MysteryBoxPage() {
  ...
  const fetchData = async () => {
    try {
      const [productsRes, boxesRes] = await Promise.all([
        ...
        fetch("/api/marketing/mystery-box", { credentials: "include" }),
      ]);
  ...
  const handleToggle = async (id: string, enabled: boolean) => {
    try {
      const res = await fetch(`/api/marketing/mystery-box/${id}`, {
        method: "PUT",
      ...
      setBoxes((prev: any[]) =>
        prev.map((box) => (box.id === id ? { ...box, enabled } : box))
      );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setBoxes((prev: any[]) => prev.filter((box) => box.id !== id));
  ...
  const handleEdit = (box: any) => {
    const productIds = box.products?.map((bp: any) => bp.productId) || [];
    const selectedItems = box.products?.map((bp: any, index: number) => ({
    ...
    setFormData({
      ...box,
      startDate: box.startDate ? new Date(box.startDate).toISOString().split('T')[0] : "",
      endDate: box.endDate ? new Date(box.endDate).toISOString().split('T')[0] : "",
      productIds,
    });
  ...
  const handleSave = async () => {
    ...
    if (formData.originalValue <= formData.price) {
      toast.error("Original value must be greater than price");
      return;
    }
    ...
    try {
      ...
      const url = editingId ? `/api/marketing/mystery-box/${editingId}` : "/api/marketing/mystery-box";
      ...
      if (editingId) {
        setBoxes((prev: any[]) => prev.map((box) => (box.id === editingId ? data : box)));
        toast.success("Updated successfully");
      } else {
        setBoxes([...boxes, data]);
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Manage surprise bundles</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-background rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <div>
                <h2 className="text-2xl font-bold">{editingId ? "Edit Mystery Box" : "Create Mystery Box"}</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">Original Value (MAD) *</label>
                      <input
                        type="number"
                        value={formData.originalValue}
                        onChange={(e) => setFormData({ ...formData, originalValue: parseFloat(e.target.value) })}
                        className="w-full px-4 py-2 rounded-lg border border-border"
                        min="0"
                        step="0.01"
                      />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData({ ...formData, stockRemaining: parseInt(e.target.value) })}
                        className="w-full px-4 py-2 rounded-lg border border-border"
                        min="0"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <ImageUpload
                      ...
                      onChange={(url: string) => setFormData({ ...formData, heroImage: url })}
                      folder="mystery-box"
                      aspectRatio="square"
                    />
                  </div>
            ...
            <div className="flex gap-3 p-6 border-t border-border">
              ...
              <button type="button" onClick={handleSave} disabled={saving || formData.price <= 0 || formData.originalValue <= formData.price} className="flex-1 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground">
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">{box.slug}</p>
              </div>
              <button onClick={() => handleToggle(box.id, !box.enabled)} className="p-2 hover:bg-muted rounded-lg">
                {box.enabled ? <ToggleRight className="w-5 h-5 text-green-500" /> : <ToggleLeft className="w-5 h-5 text-gray-400" />}
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="font-bold text-lg">{box.price.toFixed(2)} MAD</div>
              </div>
              <div className="text-right">
                <div className="text-sm text-muted-foreground">Worth</div>
                <div className="font-bold text-lg text-green-600">{box.originalValue.toFixed(2)} MAD</div>
              </div>
            </div>

            <div className="flex items-center gap-2 text-sm">
              <Gift className="w-4 h-4 text-muted-foreground" />
              <span className="font-medium">{box.products?.length || 0} items</span>
            </div>

            {box.stockLimit && (
              <div className="flex items-center gap-2 text-sm">
                <span className="text-muted-foreground">Stock:</span>
                <span className="font-medium">{box.stockRemaining}/{box.stockLimit}</span>
              </div>
            )}

            <div className="flex gap-2">
              <button onClick={() => handleEdit(box)} className="flex-1 btn-outline py-2 flex items-center justify-center gap-2">
                <Edit className="w-4 h-4" />
                Edit
              </button>
              <button onClick={() => handleDelete(box.id)} className="flex-1 btn-outline py-2 flex items-center justify-center gap-2 text-red-500">
