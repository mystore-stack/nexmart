...
import {
  Save, Plus, Trash2, Edit, ArrowLeft, Tag,
  ToggleLeft, ToggleRight, X, Image as ImageIcon
} from "lucide-react";
import toast from "react-hot-toast";
import { ImageUpload } from "@/components/admin/ImageUpload";

export default function BuyMoreSaveMorePage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/marketing/buy-more-save-more", { credentials: "include" });
  ...
  const handleToggle = async (id: string, enabled: boolean) => {
    try {
      const res = await fetch(`/api/marketing/buy-more-save-more/${id}`, {
        method: "PUT",
  ...
  const handleAdd = () => {
    setFormData({
      ...
      seoDescription: "",
      ogImage: "",
      rules: [{ minQuantity: 2, discountType: "PERCENTAGE", discountValue: 10, applicableCategories: [] }],
    });
    setEditingId(null);
    setShowForm(true);
  };
  ...
  const handleAddRule = () => {
    setFormData({
      ...formData,
      rules: [...formData.rules, { minQuantity: 2, discountType: "PERCENTAGE", discountValue: 10, applicableCategories: [] }],
    });
  };
  ...
  const handleSave = async () => {
    ...
    if (formData.rules.length === 0) {
      toast.error("At least one discount rule is required");
      return;
    }
    ...
    try {
      ...
      const url = editingId ? `/api/marketing/buy-more-save-more/${editingId}` : "/api/marketing/buy-more-save-more";
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Buy More Save More</h1>
          <p className="text-muted-foreground">Manage volume discount rules</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-background rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium">Discount Rules *</label>
                  <button type="button" onClick={handleAddRule} className="px-4 py-2 btn-primary text-sm flex items-center gap-2">
                    <Plus className="w-4 h-4" />
                    Add Rule
                  </button>
                </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 flex-1">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => handleRuleChange(index, "minQuantity", parseInt(e.target.value))}
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => handleRuleChange(index, "discountValue", parseFloat(e.target.value))}
                              className="w-full px-3 py-2 rounded border border-border text-sm"
                            />
                          </div>
                        </div>
                        <button type="button" onClick={() => handleRemoveRule(index)} className="p-2 text-red-500 hover:bg-red-50 rounded">
            ...
            <div className="flex gap-3 p-6 border-t border-border">
              ...
              <button type="button" onClick={handleSave} disabled={saving || formData.rules.length === 0} className="flex-1 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground">
        ...
        {bundles.map((bundle: any, index: number) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium">{bundle.rules?.length || 0} rules</span>
            </div>
