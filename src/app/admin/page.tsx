      ...
      try {
        ...
        newEventSource.onerror = (error) => {
          ...
          if (retryCount >= maxRetries) {
            console.log("[SSE] Max retries reached, switching to polling");
            eventSourceRef.current = null;
            // Switch to polling as fallback
            startPolling();
          } else {
            console.log(`[SSE] Retry attempt ${retryCount}/${maxRetries} in 3 seconds...`);
        ...
        newEventSource.addEventListener("error", (event) => {
          try {
            ...
            if (data.code === "NO_SESSION") {
              toast.error("Please log in to enable live updates");
            } else if (data.code === "FORBIDDEN") {
              toast.error("Admin access required for live updates");
            }
            startPolling();
          } catch (parseError) {
        ...
        newEventSource.addEventListener("orders", (event) => {
          const data = JSON.parse(event.data);
        ...
        newEventSource.addEventListener("inventory", (event) => {
          const data = JSON.parse(event.data);
        ...
        // If EventSource creation fails, immediately switch to polling
        startPolling();
      }
    };
        ...
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium ${
            ...
            <div className={`w-2 h-2 rounded-full ${
              ...
              connectionType === "polling" ? "bg-amber-500" : "bg-red-500"
            }`} />
            {isConnected ? "Live Updates" : connectionType === "polling" ? "Polling Mode" : "Disconnected"}
          </div>
          <button
            onClick={() => {
              setLoading(true);
