...
export default function AdminProductsPage() {
  ...
  const [sourceFilter, setSourceFilter] = useState<"all" | "MANUAL" | "ALIEXPRESS" | "CSV" | "DEMO">("all");
  ...
  const batchPublish = async (published: boolean) => {
    ...
    if (!confirm(`Are you sure you want to ${published ? 'publish' : 'unpublish'} ${selectedIds.size} products?`)) return;

    try {
      const res = await fetch('/api/admin/products/batch-publish', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productIds: Array.from(selectedIds), published }),
      });
      ...
      if (res.ok) {
        ...
        setSelectedIds(new Set());
        toast.success(`${selectedIds.size} products ${published ? 'published' : 'unpublished'}`);
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Link href="/admin/products/sourcing" className="btn-outline border-primary text-primary hover:bg-primary/10 py-2.5 px-5 flex items-center gap-2">
            <Package className="w-4 h-4" />
            Import Product
          </Link>
          <Link href="/admin/products/new" className="btn-primary py-2.5 px-5 flex items-center gap-2">
          ...
          {selectedIds.size > 0 && (
            <div className="flex gap-2 ml-auto">
              ...
              <button
                ...
                className="bg-secondary text-secondary-foreground px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-secondary/80"
              >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {product.source && (
                                <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 rounded uppercase font-semibold">
                                  {product.source}
                                </span>
                              )}
                            </div>
