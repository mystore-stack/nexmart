...
import {
  ...
  Package, Tag, DollarSign, Hash, Layers, Eye, EyeOff, Loader2
} from "lucide-react";
...
export default function EditProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    images: [] as File[], imageUrls: [] as string[], tags: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      form.imageUrls.forEach((url, idx) => {
        formData.append(`existingImage_${idx}`, url);
      });
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-card border border-border rounded-2xl p-6 space-y-4">
          ...
          {variants.length > 0 && (
            <div className="space-y-3">
              {variants.map((v, idx) => (
                <div key={idx} className="grid grid-cols-2 md:grid-cols-06 gap-2 p-3 rounded-xl border border-border bg-muted/30">
                  <select
                    value={v.name}
