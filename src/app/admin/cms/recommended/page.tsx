"use client";
// src/app/admin/cms/recommended/page.tsx — Recommended Products Management with Product Selector
...
export default function AdminRecommendedPage() {
  ...
  const fetchSectionProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/cms/homepage-sections/recommended/products");
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
              <Sparkles className="w-6 h-6 text-purple-600" /> Gestion des Produits Recommandés
            </h1>
            <p className="text-xs text-muted-foreground">Sélectionnez les produits à afficher dans la section "Recommended For You".</p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <SectionSettings sectionKey="recommended" onSettingsChange={handleSettingsChange} />
        </div>
      </div>
      ...
      {loading ? (
        <div className="bg-card p-12 rounded-3xl border border-border text-center space-y-3">
          ...
          <p className="font-bold text-foreground">Chargement des produits recommandés...</p>
        </div>
      ) : (
        <ProductSelector
