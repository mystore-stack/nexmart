"use client";
// src/app/deals/buy-more-save-more/page.tsx
import React, { useState, useEffect } from "react";
import { Percent, ShoppingBag, Heart, Star } from "lucide-react";
...
export default function BuyMoreSaveMorePage() {
  ...
  useEffect(() => {
    fetch("/api/homepage/sections/buyMoreSaveMore/products")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
      ...
      .catch((error) => {
        console.error("Error fetching buy more save more products:", error);
      })
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20 backdrop-blur-sm">
              <Percent className="h-6 w-6 fill-white" />
            </div>
            <h1 className="text-4xl font-bold tracking-tight">Achetez Plus, Économisez Plus</h1>
          </div>
          <p className="text-white/90 text-lg">Offres cumulatives et réductions progressives</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            <Percent className="w-16 h-16 mx-auto mb-4 text-muted-foreground opacity-50" />
            <h2 className="text-xl font-bold text-foreground mb-2">Aucune offre cumulative disponible</h2>
            <p className="text-muted-foreground">Revenez plus tard pour découvrir nos offres cumulatives</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
