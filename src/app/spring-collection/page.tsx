"use client";
// src/app/spring-collection/page.tsx — Spring Collection Standalone Page
...
export default function SpringCollectionPage() {
  ...
  const handleAddToCart = (item: NewItem) => {
    const fullProduct: Product = {
      ...
      images: [item.image],
      categoryId: "cat-1",
      category: { id: "cat-spring", name: "Collection Printemps", slug: "spring-collection" },
      tags: ["spring"],
      sku: item.id,
      stock: 15,
      lowStockAt: 2,
    ...
    <div className="min-h-screen bg-background pb-16 space-y-8">
      <div className="container-main pt-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span className="text-foreground font-bold">Collection Printemps</span>
        </div>
      </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-teal-800 via-emerald-800 to-teal-900 text-white p-8 md:p-12 shadow-luxury border border-teal-500/20">
          ...
          <div className="relative z-10 space-y-4 text-center md:text-left">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/20 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-white backdrop-blur">
              <PackagePlus className="h-4 w-4" /> Saisonnier
            </span>
            ...
            <p className="text-sm text-white/80 max-w-xl">
              Découvrez les dernières tendances pour un printemps stylé avec NexMart.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container-main">
        {loading ? (
          <div className="bg-card p-12 rounded-3xl border border-border text-center space-y-3">
            ...
            <p className="font-bold text-foreground">Chargement de la collection...</p>
          </div>
        ) : newItems.length === 0 ? (
          <div className="bg-card p-12 rounded-3xl border border-border text-center space-y-3">
            <PackagePlus className="w-10 h-10 text-muted-foreground mx-auto" />
            <p className="font-bold text-foreground">Aucun article de collection disponible pour le moment.</p>
          </div>
        ) : (
