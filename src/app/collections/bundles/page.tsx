"use client";
// src/app/collections/bundles/page.tsx
import React, { useState, useEffect } from "react";
import { Layers, ShoppingBag, Heart, Star } from "lucide-react";
...
export default function BundlesCollectionPage() {
  ...
  useEffect(() => {
    fetch("/api/homepage/sections/bundleBuilder/products")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20 backdrop-blur-sm">
              <Layers className="h-6 w-6 fill-white" />
            </div>
            <h1 className="text-4xl font-bold tracking-tight">Bundles & Packs</h1>
          </div>
          <p className="text-white/90 text-lg">Créez votre pack personnalisé et économisez</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold text-foreground mb-2">Aucun bundle disponible</h2>
            <p className="text-muted-foreground">Revenez plus tard pour découvrir nos packs</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
