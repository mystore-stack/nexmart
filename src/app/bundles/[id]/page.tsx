...
async function getBundleDeal(id: string) {
  ...
  console.log('[BUNDLE_DEAL_DETAIL] Fetching bundle', { id, organizationId, now });

  // Validate UUID format
  if (!id || id.length !== 36 || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)) {
    console.error("[BUNDLE_DEAL_DETAIL] Invalid UUID format:", id);
    return null;
  }

  console.log('[BUNDLE_DEAL_DETAIL] UUID format valid, querying database');

  // First, try to get the bundle without filters to see if it exists
  const bundleWithoutFilters = await prisma.bundleDeal.findUnique({
    where: { id },
    include: {
      products: {
        include: {
  ...
  if (!bundleWithoutFilters) {
    console.error('[BUNDLE_DEAL_DETAIL] Bundle does not exist in database at all');
    return null;
  }

  console.log('[BUNDLE_DEAL_DETAIL] Bundle exists, checking filters:', {
    bundleId: bundleWithoutFilters.id,
    organizationId: bundleWithoutFilters.organizationId,
    expectedOrgId: organizationId,
    isVisible: bundleWithoutFilters.isVisible,
    isPublished: bundleWithoutFilters.isPublished,
  });

  // Log which filters would fail
  const filterFailures = [];
  
  if (bundleWithoutFilters.organizationId !== organizationId) {
    filterFailures.push('organizationId mismatch');
  }
  if (!bundleWithoutFilters.isVisible) {
    filterFailures.push('isVisible is false');
  }
  if (!bundleWithoutFilters.isPublished) {
    filterFailures.push('isPublished is false');
  }
  if (filterFailures.length > 0) {
    console.warn('[BUNDLE_DEAL_DETAIL] Bundle would fail strict filters:', filterFailures);
    console.warn('[BUNDLE_DEAL_DETAIL] Loading bundle anyway for debugging purposes');
  } else {
    console.log('[BUNDLE_DEAL_DETAIL] Bundle passes all filters');
  }

  return bundleWithoutFilters;
}
